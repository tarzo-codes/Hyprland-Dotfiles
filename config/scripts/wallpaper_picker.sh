...
")
    ...
    # Update Hyprland border colors live with 2 Wallust colors
    if command -v hyprctl &>/dev/null && [ -n "${COLOR4:-}" ]; then
      COLOR4_STRIPPED="${COLOR4#\#}"
      COLOR5_STRIPPED="${COLOR5#\#}"
      COLOR0_STRIPPED="${COLOR0#\#}"
      hyprctl keyword "general:col.active_border"   "rgba(${COLOR4_STRIPPED}ff) rgba(${COLOR5_STRIPPED}ff) 45deg" 2>/dev/null || true
