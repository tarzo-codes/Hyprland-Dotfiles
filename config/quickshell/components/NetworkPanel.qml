...
PanelWindow {
    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            Process {
                id: refreshProc
                command: ["bash", "-c", "nmcli -t -f DEVICE,TYPE,STATE,CONNECTION dev 2>/dev/null | while IFS=: read -r dev type state conn; do ip=$(nmcli -g IP4.ADDRESS dev show \"$dev\" 2>/dev/null | head -n1 | cut -d/ -f1); echo \"DEV|$dev|$type|$state|$conn|$ip\"; done; echo '---'; nmcli -t -f IN-USE,SSID,SIGNAL,SECURITY dev wifi list 2>/dev/null"]
                running: true
                    ...
                    onStreamFinished: {
                        ...
                        if (parts.length >= 2) {
                            ...
                            for (var d = 0; d < devLines.length; d++) {
                                var line = devLines[d].trim();
                                if (line.startsWith("DEV|")) {
                                    var fields = line.split("|");
                                    var type = fields[2];
                                    var state = fields[3];
                                    var connName = fields[4] || "Ethernet";
                                    var ip = fields[5] || "";

                                    if (type === "ethernet") {
                                        networkPanel.wiredConnected = (state === "connected");
                                        networkPanel.wiredName = connName !== "" ? connName : "Ethernet";
                                        networkPanel.wiredIp = (state === "connected" && ip !== "") ? ip : "Disconnected";
                                    }
                                }
                            }
